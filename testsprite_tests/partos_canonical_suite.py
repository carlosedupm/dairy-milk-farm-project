"""
Suite canônica do fluxo de Partos para execução local estável.

Motivo: o generateCodeAndExecute do TestSprite pode sobrescrever TC*.py
com credenciais inválidas (ex.: admin@example.com). Esta suíte usa as
credenciais seed corretas e o contrato real de envelope da API.
"""
...
def run():
    ...
    status, body = api_request(
        ...
        payload={
            ...
            "numero_crias": 2,
            "crias": [
                {"sexo": "F", "condicao": "VIVO", "animal_identificacao": cria_ident},
    ...
    assert status == 200 and any(x["id"] == parto_id for x in body.get("data", [])), body
    ...
    assert status == 200 and any(a.get("identificacao") == cria_ident for a in body.get("data", [])), body
    ...
    status, body = api_request("GET", "/api/v1/animais", token=token, query={"fazenda_id": fazenda_id})
    assert status == 200 and not any(a.get("identificacao") == cria_ident for a in body.get("data", [])), body
