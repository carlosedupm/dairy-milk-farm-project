...
2. ✅ **Folgas — UX mobile refatorada mantendo grade mensal**: topo reorganizado com barra compacta (mês + ações), seções de Alertas/Equidade colapsáveis no mobile, células do calendário com resumo visual e ação única “Ver detalhes”, painel de detalhes por dia (Dialog touch-friendly) com ações contextuais (Alterar/Justificar), e histórico em cards no mobile com tabela mantida no desktop.
3. ✅ **RBAC configurável por perfil**: `appAccess.ts` + `RouteAccessGuard` no frontend; `RequirePerfilAPIAccess` no backend; FUNCIONARIO com acesso apenas a Folgas no menu e APIs permitidas (alinhado à matriz).
4. ✅ **Folgas — UX gestão e fazenda**: Filtro opcional por funcionário na página `/folgas` (destaque dos dias de folga do selecionado); ADMIN/DEVELOPER em folgas restringidos às fazendas de `/me/fazendas` com seleção automática quando há só uma; `FazendaContext` com regra explícita para 0 / 1 / N fazendas vinculadas.
...
9. ✅ **Admin Usuários — padrão de componentes e perfis**: campo perfil na rota `/admin/usuarios` padronizado com `Select` Shadcn; suporte ao perfil `GERENTE` no formulário e na listagem; correção de fallback indevido para `USER` na edição.
10. ✅ **Auto-vínculo de fazenda única no backend**: nova rotina idempotente para vincular usuário à única fazenda do sistema quando o usuário ainda não possui vínculo (aplicada em register, criação via admin e autenticação/validação).
