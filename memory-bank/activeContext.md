...
- **Assistente em linguagem natural (com contexto)**: Usuário pode escrever ou falar a necessidade (fazendas: cadastrar, listar, buscar, editar, excluir; **animais**: consultar quantidade por fazenda, listar por fazenda, detalhar, **cadastrar animal** (ex.: "cadastrar vaca Vaca 01" ou "cadastrar vaca Vaca 01 na fazenda Larissa"), **editar animal** (ex.: "alterar raça do animal X"), **excluir animal**, **registrar produção de leite** (ex.: "registrar 15 litros do animal Vaca 01")); sistema interpreta com Gemini (intent + payload) **com contexto do usuário logado** (nome, perfil) e **fazendas vinculadas ao usuário** (GetByUsuarioID) injetados no prompt. Quando o usuário tem **apenas uma fazenda** vinculada e não menciona fazenda em cadastrar_animal, listar_animais_fazenda ou consultar_animais_fazenda, o LLM inclui fazenda_id no payload e o backend usa fallback (resolveFazendaForUser) para não pedir fazenda. **Contexto condicional (economia de tokens)**: em Interpretar o prompt só inclui a lista completa de fazendas quando o usuário tem 2 ou mais; com 0 ou 1 fazenda envia apenas uma frase curta, reduzindo tokens enviados ao LLM. Validação de acesso: fazenda resolvida deve estar na lista do usuário. Backend: `Interpretar` usa GetByUsuarioID; `Executar` passa userID a executarCadastrarAnimal, executarConsultarAnimaisFazenda, executarListarAnimaisFazenda; `resolveFazendaForUser` para fallback uma fazenda e validação. `POST /api/v1/assistente/interpretar`, `POST /api/v1/assistente/executar` (requer GEMINI_API_KEY). Frontend: assistente **apenas no Header** (botão "Assistente" que abre Dialog com AssistenteInput), acessível em qualquer página. Dialog de confirmação (Shadcn), entrada por voz (Web Speech API, pt-BR) com botão de microfone. **Voz**: desktop usa modo contínuo (`continuous: true`); **Chrome Android** usa `continuous: false` + pre-warm com `getUserMedia` (workaround para falhas de interpretação — a Web Speech API no Android tem suporte limitado). Finalização por clique no microfone ou timeout de silêncio (2,5 s). **Retorno em voz (TTS)**: quando a entrada foi por voz, o sistema anuncia o resumo, erro ou sucesso. **Confirmação por voz**: no dialog de confirmação, o usuário pode dizer "sim"/"não" por voz. Recurso opcional; melhor experiência online.
...
**Contexto Ativo**: Go + Next.js 16 | Backend (Render) + Frontend (Vercel) em produção | Vínculo usuário–fazenda (usuarios_fazendas, minhas fazendas, fazenda única automática, admin atribui fazendas, perfil não editável para ADMIN/DEVELOPER) | Assistente com fazendas vinculadas ao usuário e fallback uma fazenda (cadastrar animal, listar/consultar animais por fazenda) | UX e Acessibilidade | Dev Studio Fase 0–3 | Módulo Administrador | Frontend responsivo e DRY
