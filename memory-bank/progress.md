...
- [x] **Assistente em linguagem natural**: Barra "O que você precisa?" no Header, interpretar (Gemini) + executar (FazendaService), dialog de confirmação, entrada por voz (Web Speech API pt-BR). **Voz em modo contínuo**: reconhecimento contínuo, acúmulo de transcrição, finalização por clique no microfone ou timeout de silêncio (2,5 s); suporte a diferentes velocidades de fala e pausas. **Retorno em voz (TTS)**: quando a entrada foi por voz, o sistema anuncia em voz (SpeechSynthesis pt-BR) o resumo após interpretar e a mensagem de sucesso ou erro após executar. **Confirmação por voz**: no dialog, o usuário pode dizer "sim" ou "não" por voz para confirmar ou cancelar. Persistência na edição (repository RowsAffected + ID), erro exibido dentro do dialog com mensagem detalhada (error.details)
