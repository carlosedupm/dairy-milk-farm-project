...
- **Folgas — componentes e formulários**: `frontend/src/components/folgas/` — `folgas-utils.ts` (`toYMD`, `parseApiDate`), `folgas-rodizio-utils.ts` (rótulo/divergência previsto vs registrado), `folgas-cell-tooltip.ts` (tooltip por célula, foco desktop), `FolgasCalendarioDia.tsx` (célula compacta com resumo do dia, badge “Fora do rodízio” e ação única **Ver detalhes**), `FolgasDiaDetalhesDialog.tsx` (detalhes touch-friendly por dia com previsão do rodízio, registros e ações contextuais), `FolgasHistoricoTable.tsx` (cards no mobile e tabela Shadcn no desktop). Na página: painel **Equidade no mês** (informativo) + aviso âmbar se houver Δ entre registradas e previstas; ao **substituir** dia, confirmação extra se divergir do previsto (sem bloqueio no backend). **Tratamento de conflito**: erros de duplicidade do banco (`unique_violation` / “duplicate key”) são convertidos para mensagem orientativa (ex.: indicar diferença entre `Substituir o dia inteiro` vs `Adicionar outra folga`). **DatePicker** para data âncora; **Labels** / **DialogDescription** `text-base text-muted-foreground`; ações principais **`size="lg"`**.
