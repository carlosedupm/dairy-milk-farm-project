...
- **Implementação**: `Header.tsx` — estado `mobileMenuOpen`, overlay + painel; **desktop**: busca animal (se permitido), `ThemeToggle`, **`Popover` (Radix) «Conta»** — gatilho numa **única linha** (`h-9`: iniciais + nome truncado + `ChevronDown`, `aria-label` com utilizador e fazenda); conteúdo alinhado a `end`, `max-h` + scroll: `UserIdentitySummary` panel, `FazendaSelector` (`density="drawer"`), «Nova fazenda» (titular), **Sair** (logout só no painel desktop para manter a barra compacta); **barra mobile**: busca + **avatar de iniciais** (entre busca e menu) com `aria-label`/`title` via `userIdentityAriaLabel` + botão menu; **drawer**: secção **«Conta e fazenda»** no topo, depois tema, busca animal, links e Sair. Fechar ao clicar no overlay ou ao navegar. O assistente multimodal não fica no Header (`AssistenteFab` / `AssistenteDialog` no layout).
