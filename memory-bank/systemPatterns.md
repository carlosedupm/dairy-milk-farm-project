...
- **Paginação**: usar `ListPaginationBar` (`frontend/src/components/ui/pagination.tsx`) com `total`, `pageSize`, `offset` e callbacks; opcional troca de tamanho de página. APIs offset/limit — ver `listPaginated` / `listByFazendaPaginated` em `services/animais.ts`. **Exceção — timeline da ficha**: `AnimalTimelineSection` usa `useInfiniteQuery` + Intersection Observer (`GET /api/v1/animais/:id/timeline`, chips `todos|ciclo|saude|alertas`, scroll da página); invalidar via `invalidateAnimalTimeline` em `services/animais.ts`. Para debounce em busca (ex.: identificação), usar `useDebouncedValue` (`hooks/useDebouncedValue.ts`). Na **listagem global** `/animais`, o parâmetro `fazenda_id` deve vir **apenas** de `useFazendaAtiva()` (`FazendaContext` / seletor do header), sem segundo controle de fazenda na toolbar — alinhado ao escopo “tudo ligado à fazenda ativa”.
- **Busca rápida por animal (identificação)**: um único **`AnimalSearchPanel`** (`components/animais/AnimalSearchPanel.tsx`) — debounce ~400 ms, **Enter** dispara busca imediata no mesmo `<form>`, contador em **`useRef`** para descartar respostas HTTP obsoletas quando o termo muda rápido — no **header global** (`AnimalSearchHeaderField` + `AnimalSearchDialogContext` em `Providers.tsx`) e atalho **«Buscar animal»** no `Dashboard` (mobile); **sem** card duplicado no corpo da home (desktop). **Header**: campo fixo «Brinco ou nome»; **desktop (`lg+`)** — input no header + resultados em **Popover** (`variant="header"`, input oculto no painel, termo controlado pelo header); **mobile** — input compacto na barra abre **Dialog** com painel completo (`autoFocus` no campo do diálogo). **Reset ao navegar**: `Header` passa `key={pathname}` a `AnimalSearchHeaderField` para remontar o campo a cada rota (limpa termo/overlays sem `setState` em `useEffect`). API do contexto: `openSearch()` / `registerSearchField` (atalhos e item do drawer «Ir para busca no topo»). Classes partilhadas: `animalSearchOverlay.ts`. Serviços: `searchByIdentificacao` + `getContexto` em `services/animais.ts`.
...
- **Catálogo global de fazendas (API)**: `GET /api/v1/fazendas` (raiz), pesquisas (`/search/*`), `GET /count` e `GET /exists` exigem **`RequireAdmin()`** (ADMIN ou DEVELOPER). Utilizadores não-admin listam apenas **as suas** fazendas via `GET /api/v1/me/fazendas`.
...
- **Backend**: `backend/internal/auth/perfil_access.go` — `PerfilTemAcessoAPICompleta` é falso para **FUNCIONARIO** e **USER**. `RequirePerfilAPIAccess()` aplica whitelist: para **USER**, apenas prefixo `/api/v1/me/`; para **FUNCIONARIO**, conjunto documentado em `requestAllowedForFuncionario` (folgas, restricões de leite, animais GET, gestão, etc.); demais endpoints retornam 403. Manter regras alinhadas ao TypeScript.
...
- **Rótulo em listagens (`AnimalGestaoLabel`)**: `components/gestao/AnimalGestaoLabel.tsx` — identificação + badge **«Baixado»** quando `isAnimalForaDoRebanho`; usado nas tabelas de Gestão, `ProducaoTable` e links em `PartoEditCriasPanel`. `useAnimaisMap` (string-only) permanece **deprecated**.
...
- **Campos de data**: Quando for apenas **data** (ex.: início de lactação, data de secagem, **data de fundação da fazenda**, **plantio/colheita**, custos/produções/receitas agrícolas, **análises de solo**), usar **DatePicker** (`components/ui/date-picker`) com `value`/`onChange` em `YYYY-MM-DD`. **Não** usar `Input type="date"`. Para cenários com datas históricas (ex.: cadastro de animal antigo), preferir `manualInput` no DatePicker (autoformatação `DD/MM/AAAA` durante a digitação, aceitando apenas números) + seleção por ano no calendário. Quando for **data e hora** em **parto**, **cio**, **cobertura**, **toque** ou **produção**, usar **`DateTimePickerPtBr`** para UI em português (`dd/MM/yyyy às HH:mm` + calendário pt-BR); para valor inicial a partir da API (RFC3339/UTC), usar `toDatetimeLocalInputValue` em `lib/format.ts` — **não** usar `.slice(0, 16)` na string ISO. Para “agora”, usar `nowDatetimeLocalInputValue()`. **Overlay (Dialog unificado)**: `DatePicker` e `DateTimePickerPtBr` abrem **`DatePickerOverlay`** (`date-picker-overlay.tsx`) — **Dialog em todos os breakpoints** (sem Popover); cabeçalho com título + resumo do valor; corpo scrollável; datetime com **Concluir** fixo no rodapé. Conteúdo em **`DatePickerPanel`** (`date-picker-panel.tsx`): calendário + **`<select>` nativos** para hora/minuto (evita portal Radix aninhado). **`Calendar`** (`calendar.tsx`) + overrides `.rdp` em `globals.css`: células ≥ 44px, `min-w-[20rem]`, dropdowns de mês/ano visíveis.
- **Limites temporais em eventos (BR-CICLO-012)**: `DatePicker` / `DateTimePickerPtBr` aceitam `minDate` / `maxDate` (`YYYY-MM-DD`). Em **registos de ciclo** (gestão, produção, baixa, cadastro animal, restrição `inicio_em`), passar `maxDate={todayISODate()}` de `frontend/src/lib/date-limits.ts` (datetime: calendário até hoje + selects de hora/minuto limitados quando o dia selecionado é hoje; `manualInput` rejeita data > `maxDate`). **Não** aplicar `maxDate=hoje` em filtros de listagem (`CoberturasListToolbar`, `ToquesListToolbar`, `/producao` intervalo) — consultas podem incluir datas futuras próximas. Erros TMP-* da API: `details.conformidade` + `getApiErrorMessage` nos formulários.
...
- **Listagem de toques (dia de palpação)**: `ToquesListToolbar` — filtro por dia com **`DatePicker`** (não `Input type="date"`); query `GET /api/v1/toques?fazenda_id=&data_de=&data_ate=`; `ToqueTable` com colunas **Animal | Diagnóstico | OBS**, destaque visual para `PROTOCOLO`/medicamentos; lista só leitura → ficha do animal (`ResponsiveListContainer` + `MobileListCard`).
...
- **Listagem de coberturas com filtros**: `CoberturasListToolbar` (`components/gestao/CoberturasListToolbar.tsx`) — grid responsivo com `AnimalSelect` (fêmeas, «Todos os animais»), `Select` de tipo (`COBERTURA_TIPOS`), `DatePicker` (intervalo só quando **ambas** as datas preenchidas) e «Limpar filtros». Filtragem **client-side** via `filterCoberturas` / `hasActiveCoberturaFilters` em `lib/coberturas-filter.ts` sobre `listByFazenda`; referência de intervalo de datas semelhante a `/producao`. `CoberturaTable` recebe `hasActiveFilters` para mensagem «Nenhuma cobertura encontrada com os filtros selecionados.» Evolução futura: query params server-side se o volume por fazenda crescer.
...
- **Agrupamento** (`frontend/src/config/headerNav.ts` + `getHeaderNavGroups`): permissões continuam em `appAccess.ts`; a apresentação divide itens em **Principal** (Animais, Produção, Gestão, Folgas — filtrado pelo perfil), **Mais** (ex.: Lotes, Agricultura — `Popover` «Mais», `aria-label` «Mais opções de navegação») e **Sistema** (Fazendas, Admin, Dev Studio — `Popover` «Sistema», `aria-label` «Administração e sistema»). Popovers só renderizam se o grupo tiver ≥1 item.
- **Componentes**: `Header.tsx` (shell), `HeaderDesktopNav.tsx`, `HeaderMobileNavSections.tsx`, `HeaderNavLink.tsx`, `HeaderNavPopover.tsx`, `headerNavIcons.ts`.
- **Implementação**: `Header.tsx` — contentor `max-w-7xl`; estado `mobileMenuOpen`, overlay + painel; **desktop**: `HeaderDesktopNav` entre logo e ações; links com estilo pill (`bg-accent` quando ativo, `min-h-[44px]`); separador vertical antes dos popovers secundários; **`AnimalSearchHeaderField`** (se perfil permitir `/animais`), `ThemeToggle`, **`Popover` (Radix) «Conta»** — gatilho numa **única linha** (`h-9`: iniciais + nome truncado + `ChevronDown`); **Sair** só no painel Conta (desktop); **barra mobile**: campo de busca (`flex-1`) + avatar de iniciais + menu; **drawer**: «Conta e fazenda», tema, «Ir para busca no topo» (`openSearch`), navegação com subtítulos **Exploração** / **Registos** / **Administração**, Sair. Fechar ao clicar no overlay ou ao navegar. Assistente multimodal fora do Header (`AssistenteFab` / `AssistenteDialog`).
...
**Última atualização**: 2026-05-30 (Assistente Live: tools saúde e alertas)
