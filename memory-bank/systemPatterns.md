...
- **Implementação**: `Header.tsx` — contentor `max-w-7xl`; estado `mobileMenuOpen`, overlay + painel; **desktop**: `HeaderDesktopNav` entre logo e ações; links com estilo pill (`bg-accent` quando ativo, `min-h-[44px]`); separador vertical antes dos popovers secundários; busca animal (se permitido), `ThemeToggle`, **`Popover` (Radix) «Conta»** — gatilho numa **única linha** (`h-9`: iniciais + nome truncado + `ChevronDown`); **Sair** só no painel Conta (desktop); **barra mobile**: busca + avatar de iniciais + menu; **drawer**: «Conta e fazenda», tema, busca animal, navegação com subtítulos **Exploração** / **Registos** / **Administração**, Sair. Fechar ao clicar no overlay ou ao navegar. Assistente multimodal fora do Header (`AssistenteFab` / `AssistenteDialog`).
