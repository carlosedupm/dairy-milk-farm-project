...
- **Campos de data**: Quando for apenas **data** (ex.: início de lactação, data de secagem, **data de fundação da fazenda**, **plantio/colheita**, custos/produções/receitas agrícolas, **análises de solo**), usar **DatePicker** (`components/ui/date-picker`) com `value`/`onChange` em `YYYY-MM-DD`. **Não** usar `Input type="date"`. Para cenários com datas históricas (ex.: cadastro de animal antigo), preferir `manualInput` no DatePicker (autoformatação `DD/MM/AAAA` durante a digitação, aceitando apenas números) + seleção por ano no calendário. Quando for **data e hora** em **parto**, usar **`DateTimePickerPtBr`** para UI em português (`dd/MM/yyyy às HH:mm` + calendário pt-BR). Nos demais fluxos de gestão ainda com **`Input type="datetime-local"`** (ex.: cio, cobertura, toque): para valor inicial a partir da API (RFC3339/UTC), usar `toDatetimeLocalInputValue` em `lib/format.ts` — **não** usar `.slice(0, 16)` na string ISO. Para “agora”, usar `nowDatetimeLocalInputValue()`.
- **Edição/exclusão**: **Cios** e **Partos** têm fluxo completo (página editar + Dialog de confirmação para excluir). Próximo passo: estender para coberturas, toques e secagens usando o mesmo padrão (backend PUT/DELETE + frontend página editar e coluna Ações na tabela).
