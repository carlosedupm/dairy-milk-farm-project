#!/usr/bin/env sh
# Migrações Flyway contra Postgres do Render (URL externa).
# Use antes do deploy: Flyway no container falha no Render (EOFException).
#
# 1. Render Dashboard → ceialmilk-db → Connect → External → copiar "External Database URL"
# 2. export RENDER_EXTERNAL_DATABASE_URL='postgresql://USER:PASS@HOST:5432/DATABASE'
...
case "$DB_HOST" in *.*) ;; *) DB_HOST="${DB_HOST}.oregon-postgres.render.com" ;; esac
...
docker run --rm \
  ...
  -url="$JDBC" -user="$DB_USER" -password="$DB_PASS" \
  -locations=filesystem:/flyway/sql -baselineOnMigrate=true -baselineVersion=1 \
  migrate

echo ""
echo "✅ Migrações aplicadas. Faça o deploy no Render (Flyway no container permanece desabilitado)."
