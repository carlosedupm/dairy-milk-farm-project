...
export async function updateUsuario(
  ...
  const { data } = await api.put<ApiResponse<Usuario>>(
...
export type FazendaResumo = { id: number; nome: string };

export async function getFazendasByUsuario(
  usuarioId: number
): Promise<FazendaResumo[]> {
  const { data } = await api.get<ApiResponse<FazendaResumo[]>>(
    `/api/v1/admin/usuarios/${usuarioId}/fazendas`
  );
  return data?.data ?? [];
}

export async function setFazendasForUsuario(
  usuarioId: number,
  fazendaIds: number[]
): Promise<FazendaResumo[]> {
  const { data } = await api.put<ApiResponse<FazendaResumo[]>>(
    `/api/v1/admin/usuarios/${usuarioId}/fazendas`,
