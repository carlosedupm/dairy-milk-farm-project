...
import { OnboardingColdWizard } from '@/components/onboarding/OnboardingColdWizard'
import { OnboardingColdSummary } from '@/components/onboarding/OnboardingColdSummary'
import { PassosPerfilSoPendente } from '@/components/onboarding/PassosPerfilSoPendente'
import { FaqOnboarding } from '@/components/onboarding/FaqOnboarding'
import { isOnboardingWizardCompleted } from '@/lib/onboardingStorage'
...
function OnboardingContent() {
  ...
  const isUserColdOnboarding =
    isUserPendingProfile && fazendas.length === 0 && user?.id != null

  if (isUserColdOnboarding) {
    const wizardDone = isOnboardingWizardCompleted(user.id)
    return (
      <PageContainer variant="centered">
        {wizardDone ? (
          <OnboardingColdSummary
            userName={user.nome ?? ''}
            onLogout={() => {
              logout()
            }}
