...
6. **Filtros de listagem**: `useFilterSync` + params URL (`lib/filter-url.ts`); período `start`/`end` → **`PeriodFilter`** (`components/filters/PeriodFilter.tsx`); default 30 dias só server-side (`lib/period-filter.ts`); toolbar com `ResponsiveFiltersShell` (Dialog abaixo de `md`); título `(N de M)` via `formatListCountSuffix`.
