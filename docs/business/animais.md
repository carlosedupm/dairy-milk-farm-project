...
- **Enunciado**: `proximas_acoes[]` sugere até **quatro** ações operacionais de ciclo, ordenadas por prioridade: **Parto > Secagem > Cobertura > Toque > Produção**. «Registrar baixa» **não** entra nas sugestões (fluxo dedicado na ficha). **Toque** só quando existir cobertura há ≥15 dias sem diagnóstico (alinhado a BR-CICLO-015). **Secagem** quando (a) gestação confirmada ativa **e** secagem ainda pendente (animal não `SECA` e sem secagem vinculada à gestação — [secagens.md](./secagens.md) BR-SECAGENS-006), **ou** (b) lactação ativa **sem** gestação confirmada e animal não `SECA` (secagem operacional alinhada a BR-SECAGENS-003/005 — ex.: VAZIA após toque negativo ainda em lactação). **Cobertura** quando status é vazio, `VAZIA` ou `PARIDA` e não há gestação confirmada — **com ou sem** lactação ativa (rebreeding em lactação). Animal fora do rebanho ou macho → array vazio.
...
- **Enunciado**: Na ficha `/animais/:id`, cada evento exibido nas tabs **Histórico** e **Ciclo** (e marcos no mini-preview da sidebar) possui **link de navegação** para os detalhes completos do respectivo evento (visualização/edição), permitindo ao usuário acessar a informação relacionada. Detalhe completo em [BR-CICLO-019.md](./BR-CICLO-019.md) / [ciclo-rebanho.md](./ciclo-rebanho.md).
- **Escopo**: UI (timeline Histórico + timeline visual Ciclo + mini-preview); usa `ref_id` retornado por `GET /api/v1/animais/:id/timeline`.
- **Comportamento**: clique no título → `timelineItemHref` (`lib/animalEventoLinks.ts`); paths reais em BR-CICLO-019 §1; `ALERTA` → `/alertas`; `BAIXA` sem link; RBAC via `isPathAllowedForPerfil` (read-only quando sem PUT).
- **Implementação**: `frontend/src/lib/animalEventoLinks.ts`, `AnimalTimelineList.tsx`, `AnimalCicloTimelineVisual.tsx` (`MarcoConcluidoCard`), `AnimalCicloTimelineSection` / mini-preview.
- **Estado**: **implementado** (BRF-008).
